...
#### 93.Word2vec中的负采样是什么？
```
负采样（Negative Sampling）是Word2vec中的一种优化技术，主要用于加速模型的训练过程。其主要目标是简化Softmax函数的计算，将多分类转变为二分类，从而提升训练效率
```
...
```
图像灰度化处理是将彩色图像转换为灰度图像的过程。彩色图像通常使用RGB模型，每个像素由这三种颜色的不同强度组合而成。而灰度图像中，每个像素只包含一个表示亮度的值，即从黑（0）到白（255）之间的灰度值
